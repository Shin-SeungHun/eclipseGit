...
class MainProgram extends Frame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btnBook) {
			if (menu1BookManager == null) {
				menu1BookManager = new Menu1BookManager();
			} else {
				menu1BookManager.setVisible(true);
			}
		} else if (e.getSource() == btnMember) {
			if (menu2MemberManager == null) {
				menu2MemberManager = new Menu2MemberManager();
			} else {
				menu2MemberManager.setVisible(true);
			}
		} else if (e.getSource() == btnWrite) {
			if (menu3QnA == null) {
				menu3QnA = new Menu3QnA();
			} else {
				menu3QnA.setVisible(true);
			}
		} else if (e.getSource() == btnAdmin) {
			if (menu4Admin == null) {
				menu4Admin = new Menu4Admin();
			} else {
				menu4Admin.setVisible(false);
			}
